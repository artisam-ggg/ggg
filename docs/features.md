...
## Issue #218 — Atomic constructor deployment

Escrow creation now runs the organizer-authorized `__constructor` with the Epic 1 settlement deadline and refund configuration in the same deployment transaction. The generated TypeScript binding passes all six constructor terms into `createContractV2`; the web builder simulates and assembles that single operation before Freighter signs it. A deterministic salt derived from the tournament ID prevents a later retry from creating a second escrow. The submit route rejects legacy `initialize`, validates the signed constructor terms, salt, and Wasm hash against the draft, and deduplicates deploys per tournament and organizer so cached results cannot bypass ownership checks. A deployment attempt records only its transaction hash while awaiting confirmation; retries check that transaction first and cannot send a different deployment while its result is unknown. A rejected retry checks the chain again before discarding a pending hash, recovering a late success and retaining an unresolved hash; a rejected first broadcast clears its hash and returns the original error. The contract ID, confirmed deployment hash, deadline confirmation, and `ACTIVE` state are stored only after on-chain success. The sample environment file records the uploaded and verified Testnet constructor Wasm hash. The create form uses one signature and has no initialization recovery step. Contract, XDR, API, and UI tests cover validation, authorization, rollback, duplicate submissions, timeout reconciliation, and immediate first join. Testnet deploy and first-join evidence remains operator-gated; see `docs/verification/issue-218-testnet-evidence.md`.
...
## Issue #219 — Generalize escrow payouts to 1–10 winners

The escrow constructor now accepts 1–10 positive basis-point entries totaling 10,000. Referee-only `finalize_results` accepts an equal-length ranked winner vector, rejecting empty, oversized, mismatched, duplicate, and unregistered vectors before transfers. Checked arithmetic divides the actual escrow token balance, assigns rounding dust to rank one, and stores exact payouts for `get_reward`. New `get_players` and `get_tournament` contract reads expose registration order and a stable, bounded tournament summary. The generated TypeScript binding uses the new vector ABI; the current three-winner web builder reads each instance's executable WASM hash and retains the legacy three-address call for older contracts. App-side N-winner forms, persistence, validation, API, and subscriber handling remain in #224. The #219 app and WASM hash must not be deployed until #224 adds subscriber support for the new finalized-event vectors. Contract tests cover N=1 through N=10, the 100-player helper boundary, and a specified five-winner payout. The #219 WASM is uploaded and its Testnet `ContractCode` bytes verified; no contract instance or live payout has been tested. See `docs/verification/issue-219-payout-evidence.md`.

## Issue #220 — Add the escrow TTL lifecycle policy

Successful constructor, join, finalization, cancellation, and refund calls keep both the contract instance and code available through a bounded `extend_ttl_with_limits` policy. Current Testnet settings checked on 2026-09-15 report a five-second target ledger close, a 7-day minimum persistent TTL, and a 180-day maximum entry TTL. The contract uses a 90-day threshold (`1,555,200` ledgers) and a 120-day target (`2,073,600` ledgers), covering #215's maximum settlement horizon with a 30-day margin while staying below the network ceiling. Exactly at the threshold no extension occurs; one ledger below it triggers the bump, and one call can add at most 120 days. Read-only calls do not extend TTL, and TTL remains independent of authorization and the inclusive settlement deadline. Archived instance or code entries must be restored by transaction simulation before invocation. Contract tests advance ledger sequence and timestamp together to cover both entry types, threshold boundaries, successful mutations, failed-call rollback, reads without extension, and a refund at the maximum deadline.

The pinned #220 artifact was uploaded to Testnet and its `ContractCode` bytes verified without deploying a contract instance. The app recognizes both the #219 and #220 vector-finalization hashes; Railway remains on the legacy hash until #224 subscriber support is deployed. See `docs/verification/issue-220-ttl-evidence.md`.
