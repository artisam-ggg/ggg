...
Escrow creation now runs the organizer-authorized `__constructor` with the Epic 1 settlement deadline and refund configuration in the same deployment transaction. The generated TypeScript binding passes all six constructor terms into `createContractV2`; the web builder simulates and assembles that single operation before Freighter signs it. A deterministic salt derived from the tournament ID prevents a later retry from creating a second escrow. The submit route rejects legacy `initialize`, validates the signed constructor terms, salt, and Wasm hash against the draft, deduplicates deploys by tournament, and stores the contract ID, transaction hash, confirmed deadline, and `ACTIVE` state only after on-chain success. The create form uses one signature and has no initialization recovery step. Contract, XDR, API, and UI tests cover validation, authorization, rollback, duplicate submissions, and immediate first join. Testnet deploy and first-join evidence remains operator-gated; see `docs/verification/issue-218-testnet-evidence.md`.
